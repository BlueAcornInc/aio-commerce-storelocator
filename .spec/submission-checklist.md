...
| ------ | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | -------------- | ----------------- | -------------------------------------- |
| DOC-1  | Documentation | PaaS vs SaaS distinction in README.md — remove "SaaS" references, clarify PaaS-only steps                                                                                   | 🟡     | ✅ Done        | Copilot           | PaaS/On-Prem callout added             |
| DOC-2  | Documentation | Add `npm install` step before `aio app deploy` — already present in code block, needs prose callout                                                                         | 🟢     | ✅ Done        | Copilot           | Added prose note above code block      |
| DOC-3  | Documentation | Fix broken image link at README.md line ~154 (`docs/img/admin-ui-sdk-setup.png`)                                                                                            | 🟢     | ✅ Done        | Copilot           | Image exists & is git-tracked          |
| DOC-4  | Documentation | Fix AIO CLI plugin install command — verify no `@adobe/aio-cli-plugin-commerce` refs remain                                                                                 | 🟢     | ✅ Done        | Copilot           | Verified: GitHub URL used everywhere   |
...
| DOC-6  | Documentation | Fix inaccessible links in `blocks/store-locator/README.md` (private Google Sheets/Docs URLs)                                                                                | 🟡     | ✅ Done        | Copilot           | Replaced with generic instructions     |
...
| SEC-3  | Security          | Private key + cert in `dist/dev-keys/` — may be git-tracked                                                                   | 🟢     | ✅ Done        | Copilot      | Already gitignored via `dist` rule  |
| PROJ-1 | Project Structure | `env.dist` is empty (`# no variables are required`) — guidelines require all keys documented                                  | 🟢     | ✅ Done        | Copilot      |                                     |
