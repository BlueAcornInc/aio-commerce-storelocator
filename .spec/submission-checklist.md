...
| PROJ-3 | Project Structure | `.devcontainer/devcontainer.json` says `"ShipsStation Commerce App"` — copy-paste bug                                         | 🟢     | ⬜ Not Started |              |                                     |
| UI-1   | Admin UI          | `App.jsx` L53 — `onError(e, componentStack) {}` is empty (no logging)                                                         | 🟢     | ⬜ Not Started |              |                                     |
| UI-2   | Admin UI          | `Welcome.jsx` — says "Magento's native MSI". Guidelines say avoid "Magento"                                                   | 🟢     | ⬜ Not Started |              |                                     |
| BLK-1  | Blocks            | `store-locator.js` ~L128 — `tel:${item}` uses string `"phone"` not `store.phone`                                              | 🟢     | ⬜ Not Started |              |                                     |
