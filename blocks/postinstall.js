...
function installBlocks() {
  ...
  if (!projectRoot) {
    ...
    console.error('Please ensure you are installing this package in a project that has a blocks/ directory');
    process.exit(1);
  }
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'node_modules') {
        ...
        if (fs.existsSync(targetPath)) {
          console.log(`Warning: Block '${entry.name}' already exists. Skipping...`);
          console.log(`If you want to update, please remove the existing block first.`);
          continue;
        }
